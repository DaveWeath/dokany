...
--*/
{
  ...
  __try {
    ...
    *(PWCHAR)((char *)&eventContext->Operation.Create +
	...
	OpenRequiringOplock = BooleanFlagOn(irpSp->Parameters.Create.Options, FILE_OPEN_REQUIRING_OPLOCK);
#else
	...
	if (fcb->FileCount > 1) {
		...
		//  check for writable user sections if the user did not allow write sharing.
		//
		if (!NT_SUCCESS(status = DokanCheckShareAccess(fileObject,
			...
			if ((status == STATUS_SHARING_VIOLATION) &&
				...
				OplockBreakStatus = FsRtlOplockBreakH(DokanGetFcbOplock(fcb),
					Irp,
					0,
					eventContext,
					DokanOplockComplete,
	...
	//  we don't need to do this check; oplocks can only exist when there are handles.
	//
	...
	if (OpenRequiringOplock) {
		...
		if ((status == STATUS_SUCCESS)) {
			status = FsRtlOplockFsctrl(DokanGetFcbOplock(fcb),
				Irp,
				fcb->FileCount);
		}
  ...
  //
	if (AbnormalTermination() ||
      ...
      if (OpenRequiringOplock &&
			...
			(fcb != NULL)) {
		FsRtlCheckOplockEx(DokanGetFcbOplock(fcb),
		  Irp,
